...
void function1(){ 
	...
	External(global) static variables remain restricted to scope of file in which they are declared.*/
	//Static cannot be used out of one c file to another c file
    x=x+1;  
    y=y+1;  
...
int main()
{
    ...
    function1();
    function1();
	const volatile flag = 98; //flag is declared constant so they can’t be changed by the program. But hardware can change it as they are volatile objects.
